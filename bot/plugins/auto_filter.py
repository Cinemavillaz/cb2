...
async def auto_filter(bot, update):
    ...
    if filters:
        results.append(
                [
                    InlineKeyboardButton("🔘 JOIN OUR MAIN CHANNEL 🔘", url="https://t.me/cb_psarips1")
                ]
            )
        for filter in filters: # iterating through each files
            ...
            if file_size < 1024:
                file_size = f"{file_size} ʙ"
            elif file_size < (1024**2):
                file_size = f"{str(round(file_size/1024, 2))} ᴋʙ"
            elif file_size < (1024**3):
                file_size = f"{str(round(file_size/(1024**2), 2))} ᴍʙ"
            elif file_size < (1024**4):
                file_size = f"{str(round(file_size/(1024**3), 2))} ɢʙ"
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➡️", callback_data=f"navigate(0|next|{query})")
                ]
            )
